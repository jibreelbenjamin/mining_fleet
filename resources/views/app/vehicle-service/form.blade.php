    ...
    <div class="mt-9">
      ...
      <form method="post" action="{{ route('dashboard.'.$page.'.'.$mode.'.action', $data->$id_data ?? '') }}">
        ...
        <div class="grid gap-4 lg:gap-6">
            ...
            <div>
                <label class="block mb-2 text-sm text-gray-800 dark:text-neutral-200 font-medium">Tanggal servis</label>
                <input type="datetime-local" name="service_date" value="{{ $mode == 'update' ? $data->service_date : old('service_date') }}" autocomplete="off" placeholder="{{ $mode == 'update' ? $data->service_date : 'Pilih tanggal ...' }}" class="datetime py-2.5 sm:py-3 px-4 block w-full bg-white dark:bg-neutral-800 border-gray-200 dark:border-neutral-700 rounded-lg sm:text-sm text-gray-800 dark:text-neutral-200 placeholder:text-gray-500 dark:placeholder:text-neutral-400 focus:border-blue-700 dark:focus:border-blue-600 focus:ring-blue-700 dark:focus:ring-blue-600 disabled:opacity-50 disabled:pointer-events-none">
            </div>

            <div>
                <label class="block mb-2 text-sm text-gray-800 dark:text-neutral-200 font-medium">Keterangan</label>
                <input type="text" name="keterangan" value="{{ $mode == 'update' ? $data->keterangan : old('keterangan') }}" autocomplete="off" placeholder="{{ $mode == 'update' ? $data->keterangan : 'Ganti oli, Perbaikan berkala, dll ...' }}" class="py-2.5 sm:py-3 px-4 block w-full bg-white dark:bg-neutral-800 border-gray-200 dark:border-neutral-700 rounded-lg sm:text-sm text-gray-800 dark:text-neutral-200 placeholder:text-gray-500 dark:placeholder:text-neutral-400 focus:border-blue-700 dark:focus:border-blue-600 focus:ring-blue-700 dark:focus:ring-blue-600 disabled:opacity-50 disabled:pointer-events-none">
            </div>

            <div>
                <label class="block mb-2 text-sm text-gray-800 dark:text-neutral-200 font-medium">Total biaya</label>
                <div class="relative">
                    <input type="number" name="cost" value="{{ $mode == 'update' ? $data->cost : old('cost') }}" autocomplete="off" placeholder="{{ $mode == 'update' ? $data->cost : '700000 ...' }}" class="py-2.5 sm:py-3 px-4 ps-10 pe-16 block w-full bg-white dark:bg-neutral-800 border-gray-200 dark:border-neutral-700 rounded-lg sm:text-sm text-gray-800 dark:text-neutral-200 placeholder:text-gray-500 dark:placeholder:text-neutral-400 focus:z-10 focus:border-blue-700 dark:focus:border-blue-600 focus:ring-blue-700 dark:focus:ring-blue-600 disabled:opacity-50 disabled:pointer-events-none" >
